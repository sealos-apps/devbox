...
func TestGenerateEnvProfile(t *testing.T) {
	tests := []struct {
		name       string
		envs       []corev1.EnvVar
		kubeAccess *devboxv1alpha2.KubeAccessSpec
		want       string
	}{
		{
		...
		{
			name:       "kube access enabled adds kubeconfig env",
			kubeAccess: &devboxv1alpha2.KubeAccessSpec{Enabled: true},
			want: "# Generated by Sealos Devbox\n" +
				"export DEVBOX_JWT_SECRET=\"test-jwt-secret\"\n" +
				"export KUBECONFIG=\"/var/run/sealos/kubeconfig/config\"\n",
		},
	}
