...
This directory contains the `v1` CRI shim binary for the DevBox repository.
It is kept as an independent Go module under:

`github.com/sealos-apps/devbox/v1/cri-shim`

The shim implements a CRI-compatible bridge for containerd-related workflows used by DevBox.
...
This produces Linux artifacts in `bin/` for:
...
sudo ./bin/cri-shim_linux-amd64 server --cri-socket=unix:///var/run/containerd/containerd.sock --shim-socket=/var/run/sealos/cri-shim.sock
```
...
- CI should validate this module with `go test` and `make build`
- Release should publish binary artifacts instead of container images
